...
DicomVision 是一套 C/S 架构的 DICOM Viewer，面向影像浏览、重建、测量、QA、元数据审阅、序列对比与脱敏导出。软件支持 Stack、MPR/斜切 MPR、3D 体渲染、4D 时相播放、布局阅片、同步对比、DICOM Tag 修改与 Web / Windows / macOS 桌面部署。

## 核心亮点

- 支持患者 / 检查层级的序列管理、拖拽导入、标签页阅片和可配置布局。
- 支持 PACS Browser，可通过 DICOMweb 和 DIMSE 查询检查 / 序列，下载缓存后直接打开阅片。
...
- **PACS 工作流**：配置 Orthanc、dcm4chee 或自定义 PACS Profile，通过 DICOMweb QIDO 或 DIMSE C-FIND 查询检查，通过 DICOMweb WADO 或 DIMSE C-GET 下载序列，并直接打开已缓存影像。
...
- **部署形态**：Web 端可连接远程后端；桌面端可打包为内置后端的 Windows/macOS Electron 应用。
...
| --- | --- |
| <img src="./screenshots/pacs_dicom_import.png" alt="PACS DICOMweb 与 DIMSE Profile 设置" width="420"> | <img src="./screenshots/pacs_dicom_import_1.png" alt="PACS Browser 查询并打开已下载序列" width="420"> |
...
如果 `DicomVisionServer` 与当前仓库位于同级目录，可一键生成桌面端安装包：
...
生成的安装包 / App 产物位于 `dist-electron/`。运行时 Electron 主进程会在 Windows 上从 `resources/server/DicomVisionServer.exe` 启动内置服务端，在 macOS 上从 `resources/server/DicomVisionServer` 启动内置服务端，分配本地端口，并让 UI 自动连接到解析后的后端地址。macOS 产物需要在 macOS 上构建。
...
- `npm run release:mac`：在 macOS 上构建服务端桌面 bundle 并生成 DMG/ZIP 产物。
