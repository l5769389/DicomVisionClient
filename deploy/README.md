...
```text
http://111.228.1.213/
http://111.228.1.213/health
```

如果服务已经通过 HTTPS 域名访问，推荐保持 `PUBLIC_ORIGIN=same-origin`，让前端通过当前域名访问 `/api/` 和 `/socket.io/`。不要在 HTTPS 站点中把 `PUBLIC_ORIGIN` 配成 `http://...`，否则浏览器会拦截 WebSocket mixed content。
...
- `JDCLOUD_PUBLIC_ORIGIN`：可选，公网访问地址，默认 `same-origin`。

如果后续绑定域名并开启 HTTPS，可以继续使用 `same-origin`；如果需要显式配置，必须使用 HTTPS 域名，例如：
...
- 如果只通过公网 IP 测试，可以先使用 `http://111.228.1.213/`。
